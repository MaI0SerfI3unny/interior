...
const validationSchema = Yup.object({
  photo : Yup.mixed().test(
    'fileSize',
    'Файл занадто великий (максимум 150KB)',
    value => {
  ...
  style: Yup.string()
  .oneOf(initialStyleValues.map(style => style.initialValue), 'Выберите один из вариантов')
  .required('Оберіть стиль'),

  room :  Yup.string()
  .oneOf(initialRoomValues.map(room=> room.value), 'Выберите страну из списка')
    ...
    >
      {({ values, setFieldValue }) => (
        <GeneratingFormStyles>
          ...
          <PromptInput/>
          <SelectStyleInputContainer initialValues={initialStyleValues} value={values.style}/>
