          ...
          {why_me.map(({ img }, key) => (
            <div key={key}>
              <lord-icon
                ...
                className={style.whyWeIcon}></lord-icon>
                <p className={style.whyWeContainerListTitle}>{t(`whyWe.title_item_${key}`)}</p>
                <p className={style.whyWeContainerListDesc}>{t(`whyWe.desc_item_${key}`)}</p>
            </div>))}
        </div>
      </SlideOnScroll>
    </section>
